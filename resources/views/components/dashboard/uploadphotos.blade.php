<div x-data="{ imgOpen: false, existingImg: false }" x-on:close-parent-modal="showModal = false" class="relative flex justify-center z-30">
    <button @click="imgOpen = true" class="w-full px-6 py-2 mx-auto tracking-wide text-darkgreen capitalize transition-colors duration-300 transform bg-[#f8e5e5] hover:bg-[#e06469] hover:text-white"><span class="mx-auto block"><box-icon name='image-alt'></box-icon></span> Upload Image</button>
                ...
                <div class="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl rtl:text-right dark:bg-gray-900 sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
                    <div>
                        <div class="mt-2 text-center">
                            ...
                            <form id="uploadphotosform" action="{{route('uploadphoto')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="text" hidden name="userid" value="{{auth()->user()->id}}">
                                <input type="file" name="imgs[]" multiple max="5" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" id="singleimage">
                                ...
                                <div class="sm:flex sm:items-center ">
                                    ...
                                    <button type="button" class="w-full px-4 py-2 mt-2 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-blue-600 rounded-md sm:w-auto sm:mt-0 hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40" id="uploadphoto">
                                        Upload image
                                    </button>
